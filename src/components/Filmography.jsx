  ...
  return (
    <div>
        ...
        {filmography.map((item) => (
            <Link to={item.media_type === 'movie' ? `${movie}${item.id}` : `${tv}${item.id}`} ><div key={item.id} className='w-[336px] h-[227px] md:w-[386px] md:h-[257px] p-6  '>
                ...
                <h3 className=' text-base sm:text-base font-medium text-white text-center'>{item.name ? item.name : item.title}</h3>
