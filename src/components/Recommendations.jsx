...
export default function Recommendations({fetchUrl, pathto}) {
  ...
  return (
    <div className='flex flex-row flex-wrap sm:w-5/6 justify-center mx-auto mt-10'>
        {recommends.map((item) => (
            <Link to={`${pathto}${item.id}`} relative="path"><div key={item.id} className='w-[336px] h-[227px] p-6  '>
                ...
                <h3 className=' text-base sm:text-base font-medium text-white text-center'>{item.name}</h3>
