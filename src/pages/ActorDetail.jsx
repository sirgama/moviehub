          ...
          <div className="w-full h-screen">
              ...
              <div className="flex flex-row flex-wrap">
                <div className="w-[220px] sm:w-[290px]  h-auto sm:h-auto inline-block cursor-pointer relative mb-6 p-3 sm:p-0 mt-10 ml-10  sm:mt-32 sm:ml-14 sm:mr-4">
                    <img className='w-auto h-full block bg-cover bg-center rounded-lg' src={`https://image.tmdb.org/t/p/w500${actor?.profile_path && actor?.profile_path}`} alt={actor?.name} />
                </div>
                <div className="text-white flex flex-col p-3 sm:p-0 mt-1 mx-1  sm:mt-32 sm:mx-1">
                        <h1 className="font-bold underline underline-offset-8 sm:no-underline text-center md:text-start text-4xl mb-10 sm:ml-20">{actor?.name}</h1>
                        <div className="flex flex-row flex-wrap">
                                <div className="flex flex-col m-4 sm:ml-20">
                                    <h1 className="font-bold sm:text-xl my-4">Gender</h1>
                                    <h1 className="">{actor?.gender === 1 ? 'Female' : 'Male'}</h1>
