...
export default function MovieDetail({
  ...
  const getMovie = () => {
    axios
      .get(
        `https://api.themoviedb.org/3/movie/${tv_id}?api_key=${food}&language=en-US`
      )
      .then((res) => {
  ...
  const getVideos = () => {
    axios
      .get(
        `https://api.themoviedb.org/3/movie/${tv_id}/videos?api_key=${food}&language=en-US`
      )
      .then((res) => {
  ...
  const getCredits = () => {
    axios
      .get(
        `https://api.themoviedb.org/3/movie/${tv_id}/credits?api_key=${food}&language=en-US`
      )
      .then((res) => {
        ...
        <>
              ...
              <div className="absolute top-[5%] md:top-[10%] p-5 m-5 flex flex-row flex-wrap sm:flex-nowrap">
                <div>
                  <div className="w-[280px] sm:w-[240px] lg:w-[280px] h-96 sm:h-96 inline-block cursor-pointer relative p-3 md:p-0">
                    <img
                      className="w-auto h-full block bg-cover bg-center rounded-lg shadow-xl shadow-pink-900/20"
                      src={`https://image.tmdb.org/t/p/w500${
                        singleMovie?.poster_path && singleMovie?.poster_path
                      }`}
                      alt={singleMovie?.title}
                    />
                    <a
                    ...
                    >
                      <button className="border text-center inline-block md:my-4 w-[210px] md:w-[250px] p-2 my-6 md:px-4 rounded-md bg-gray-100 text-black font-semibold">
                        <AiFillYoutube className="inline" /> Play Trailer
                      </button>
                    </a>
                  </div>
                </div>
                ...
                <div className="md:mx-20 md:px-10 md:my-0 my-10">
                  ...
                  <div className="my-4">
                    <p className="text-gray-00 text-base">
                      <span className="font-semibold text-white">
                        {mvg &&
                          mvg.map((genre) => (
                            <div className="px-5 py-1 border border-white rounded-full my-1 mx-1 inline-block md:mx-2 hover:bg-gray-200 hover:text-black cursor-pointer">
                              {genre.name}
                            </div>
                          ))}
                      </span>{" "}
                      ...
                      <span className="font-semibold text-white md:text-xl">
                        {date}
                      </span>{" "}
                      <span className="text-xl font-bold p-2"> ~ </span>
                      <span className="font-semibold text-white md:text-xl">
                  ...
                  <p className="w-full text-gray-100 md:font-semibold text-sm py-2 md:text-xl">
                    {singleMovie?.tagline && singleMovie?.tagline}
                  </p>
                  <p className="w-full text-gray-100 md:font-semibold text-sm py-2 md:text-xl">
                    {singleMovie?.overview && singleMovie?.overview}
                  </p>
                  <div className="my-0">
                    <button className="text-gray-100 md:my-6 ">
                      Rating:{" "}
                      <span className="text-5xl font-semibold">
                        {singleMovie?.vote_average &&
                          Math.round(singleMovie?.vote_average)}
                      </span>
                      / 10{" "}
                    </button>
                    <button className="text-gray-100 px-2 md:px-10 ">
                      Votes:{" "}
                      <span className="text-sm font-extrabold md:text-3xl">
                        {singleMovie?.vote_count && singleMovie?.vote_count}
                      </span>
                    </button>
                  </div>
                  <div className="my-4 flex flex-row flex-wrap rounded-lg border-4">
                    <div className="text-white  py-2 mx-auto flex flex-col">
                      <button className="px-10 md:text-2xl md:my-2">
                        Status
                      </button>
                      <button className="font-semibold md:text-2xl md:my-2">
                        {singleMovie?.status && singleMovie?.status}
                      </button>
                    </div>
                    <div className="text-white  py-2 mx-auto flex flex-col">
                      <button className="px-10 md:text-2xl md:my-2">
                      ...
                      <button className="font-semibold md:text-2xl md:my-2">
                        ${singleMovie?.budget && singleMovie?.budget}
                      </button>
                    </div>
                    <div className="text-white  py-2 mx-auto flex flex-col">
                      <button className="px-10 md:text-2xl md:my-2">
                    ...
                    <div className="text-white  py-2 mx-auto  flex-col hidden sm:flex">
                      ...
                      <a href={singleMovie?.homepage && singleMovie?.homepage}>
                        <button className="font-semibold text-sm md:text-xl md:my-2">
                          {singleMovie?.homepage && (
                            <div className="flex">
                              <AiOutlineLink className="text-center text-2xl mx-20" />{" "}
                            </div>
                          )}
                        </button>
                      </a>
          ...
          <div
            className={`w-screen md:w-full ${
              singleMovie.overview ? "mt-96" : "mt-64"
            }  md:mt-64`}
          >
            <Tabs id="custom-animation" className="mt-64 md:mt-10" value="cast">
              <TabsHeader className="w-11/12 sm:w-2/3 mx-auto mt-44 md:mt-28">
                <Tab
                  value="cast"
                  className="text-gray-900 font-bold py-2 md:text-xl md:py-2 m-1"
                >
                  Cast
                </Tab>
                <Tab
