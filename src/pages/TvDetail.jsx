...
export default function TvDetail({
  ...
  const getTv = () => {
    axios
      .get(
        `https://api.themoviedb.org/3/${tv}${tv_id}?api_key=${food}&language=en-US`
      )
      .then((res) => {
  ...
  const getCredits = () => {
    axios
      .get(
        `https://api.themoviedb.org/3/${tv}${tv_id}/credits?api_key=${food}&language=en-US`
      )
      .then((res) => {
          ...
          <div className="w-full h-screen md:h-[650px] text-white">
            <div className="w-full h-full">
              ...
              <img
                ...
                alt={tvShow?.backdrop_path && tvShow?.name}
              />
                  ...
                  <div className="w-[280px] sm:w-[240px] lg:w-[280px] h-80 sm:h-96 inline-block cursor-pointer relative p-3 md:p-0">
                    <img
                      ...
                      src={`https://image.tmdb.org/t/p/w500${
                        tvShow?.poster_path && tvShow?.poster_path
                      }`}
                      alt="single "
                    />
                  </div>
                ...
                <div className="md:mx-20 md:px-10 md:my-0 mt-10">
                      ...
                      <span className="font-semibold text-white">
                        {mvg &&
                          mvg.map((genre) => (
                            <div
                              key={genre.id}
                              className="px-1 md:px-5 py-1 border-2 border-white rounded-full my-1 mx-1 inline-block md:mx-2 hover:bg-gray-200 hover:text-black cursor-pointer text-xs md:text-xl"
                            >
                              {genre.name}
                            </div>
                          ))}
                      </span>{" "}
                      <span className="text-sm md:text-xl font-bold p-2">
                        {" "}
                        ~{" "}
                      </span>{" "}
                      <br className=" sm:hidden" />
                      <span className="font-semibold text-white md:text-xl text-xs">
                        {date}
                      </span>{" "}
                  ...
                  <div className="my-0">
                    <button className="text-gray-100 md:my-6 ">
                      Rating:{" "}
                      <span className="text-3xl md:text-5xl md:px-2 font-semibold">
                        {tvShow?.vote_average &&
                    ...
                    <button className="text-gray-100 px-2 md:px-10 ">
                      Votes:{" "}
                      <span className="text-sm md:px-2 font-extrabold md:text-3xl">
                        {tvShow?.vote_count && tvShow?.vote_count}
                      </span>
                    </button>
                  </div>
                  <div className="my-4 flex flex-row flex-wrap rounded-lg border-4 items-center">
                    <div className="text-white  py-2 mx-auto flex flex-col">
                      ...
                      <button className="font-semibold text-sm md:text-xl md:px-4 md:my-2">
                    ...
                    <div className="text-white  py-2 mx-auto flex flex-col">
                      ...
                      <button className="font-semibold text-sm md:text-xl md:my-2">
                        {tvShow?.seasons && tvShow?.number_of_seasons}
                      </button>
                    </div>
                    <div className="text-white  py-2 mx-auto flex flex-col">
                      <button className="md:px-10 md:text-2xl md:my-2">
                        Episodes
                      </button>
                      <button className="font-semibold text-sm md:text-xl md:my-2">
                        {tvShow?.type && tvShow?.number_of_episodes}
                      </button>
                    </div>
                    <div className="text-white hidden py-2 mx-auto md:flex flex-col">
                      ...
                      <a href={tvShow?.homepage && tvShow?.homepage}>
                        <button className="font-semibold text-sm md:text-xl md:my-2">
                          {tvShow?.homepage && (
                            <div className="">
                              <AiOutlineLink className="text-center text-sm md:text-2xl mx-20" />{" "}
                            </div>
                          )}
                        </button>
                      </a>
